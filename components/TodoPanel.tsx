...
import { X, Plus, Trash2, CheckCircle2, Circle, ClipboardList, GripVertical } from 'lucide-react';
        ...
        <div className="flex flex-col h-full">
          <div className="p-6 border-b border-gray-100 dark:border-gray-800">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={onClose} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-colors text-gray-400">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs font-bold text-gray-400 uppercase tracking-wider">
                <span>Progress</span>
                <span>{completedCount}/{todos.length} ({Math.round(progress)}%)</span>
              </div>
          ...
          <form onSubmit={handleSubmit} className="p-4">
            <div className="relative">
              <input
                ...
                className="w-full pl-4 pr-10 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all text-sm dark:text-white"
              />
              <button
                type="submit"
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-6">
            {todos.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-40 text-center opacity-40">
                ...
                <p className="text-sm dark:text-gray-400">No tasks yet.</p>
              </div>
            ) : (
              <div className="space-y-1">
                {todos.map((todo) => (
                  <div
                    key={todo.id}
                    className="group flex items-center justify-between p-3 rounded-xl border border-transparent hover:border-gray-100 dark:hover:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-all"
                  >
                    <div className="flex items-center space-x-3 overflow-hidden">
                      ...
                      <span className={`text-sm transition-all truncate ${todo.completed ? 'text-gray-400 line-through' : 'text-gray-700 dark:text-gray-300 font-medium'}`}>
                        {todo.text}
                      </span>
                    </div>
                    <button
